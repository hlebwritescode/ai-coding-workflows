...
| `domain-modeling` | model-invoked | [mattpocock/skills](https://github.com/mattpocock/skills/tree/main/skills/engineering/domain-modeling) | Active domain-model discipline — challenge terms, write glossary (`CONTEXT.md`) and ADRs (`docs/adr/`) inline. Ships `CONTEXT-FORMAT.md` and `ADR-FORMAT.md`. Adaptations: multi-context support (`CONTEXT-MAP.md`) removed — single root `CONTEXT.md` only; three ADR criteria consolidated into `SKILL.md`, with `ADR-FORMAT.md` pointing back. |
